...
class TurtleControllerNode(Node):
    ...
    def __init__(self):
        ...
        self.__constants = self.__Constants(FORWARD_SPEED=2.0, FORWARD_TURNING_SPEED=0.3, TURNING_SPEED=0.5, VECTORS_ALIGNMENT=0.5, TURTLES_DISTANCE=0.2)
        ...
        self.__subscriber_spawned_turtle_position = self.create_subscription(Pose, "spawned_turtle", self.callback_subscription_spawned_turtle, 10)   
    ...
    def callback_subscription_turtle_position(self, turtle_postion: Pose):
        if len(self.__spawned_turtles) > 0:
            ...
            if (abs(turtle_postion.x - self.__spawned_turtles[0].x) < self.__constants.TURTLES_DISTANCE 
                and abs(turtle_postion.y - self.__spawned_turtles[0].y) < self.__constants.TURTLES_DISTANCE):
                ...
                self.get_logger().info("turtle_position_x = " + str(turtle_postion.x))
                self.get_logger().info("__prev_x = " + str(self.__prev_x))
                self.get_logger().info("vel_vec_x = " + str(vel_vec_x))

                dest_vec_x = self.__spawned_turtles[0].x - turtle_postion.x
                dest_vec_y = self.__spawned_turtles[0].y - turtle_postion.y

                self.get_logger().info("dest_vec_x = " + str(dest_vec_x))
                self.get_logger().info("__spawned_turtle_x = " + str(self.__spawned_turtles[0].x))
