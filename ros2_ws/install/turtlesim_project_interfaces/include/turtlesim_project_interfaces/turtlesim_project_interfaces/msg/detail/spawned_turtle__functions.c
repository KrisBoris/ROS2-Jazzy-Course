// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from turtlesim_project_interfaces:msg/SpawnedTurtle.idl
// generated code does not contain a copyright notice
#include "turtlesim_project_interfaces/msg/detail/spawned_turtle__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `name`
#include "rosidl_runtime_c/string_functions.h"
// Member `pose`
#include "turtlesim/msg/detail/pose__functions.h"

bool
turtlesim_project_interfaces__msg__SpawnedTurtle__init(turtlesim_project_interfaces__msg__SpawnedTurtle * msg)
{
  if (!msg) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__init(&msg->name)) {
    turtlesim_project_interfaces__msg__SpawnedTurtle__fini(msg);
    return false;
  }
  // pose
  if (!turtlesim__msg__Pose__init(&msg->pose)) {
    turtlesim_project_interfaces__msg__SpawnedTurtle__fini(msg);
    return false;
  }
  return true;
}

void
turtlesim_project_interfaces__msg__SpawnedTurtle__fini(turtlesim_project_interfaces__msg__SpawnedTurtle * msg)
{
  if (!msg) {
    return;
  }
  // name
  rosidl_runtime_c__String__fini(&msg->name);
  // pose
  turtlesim__msg__Pose__fini(&msg->pose);
}

bool
turtlesim_project_interfaces__msg__SpawnedTurtle__are_equal(const turtlesim_project_interfaces__msg__SpawnedTurtle * lhs, const turtlesim_project_interfaces__msg__SpawnedTurtle * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->name), &(rhs->name)))
  {
    return false;
  }
  // pose
  if (!turtlesim__msg__Pose__are_equal(
      &(lhs->pose), &(rhs->pose)))
  {
    return false;
  }
  return true;
}

bool
turtlesim_project_interfaces__msg__SpawnedTurtle__copy(
  const turtlesim_project_interfaces__msg__SpawnedTurtle * input,
  turtlesim_project_interfaces__msg__SpawnedTurtle * output)
{
  if (!input || !output) {
    return false;
  }
  // name
  if (!rosidl_runtime_c__String__copy(
      &(input->name), &(output->name)))
  {
    return false;
  }
  // pose
  if (!turtlesim__msg__Pose__copy(
      &(input->pose), &(output->pose)))
  {
    return false;
  }
  return true;
}

turtlesim_project_interfaces__msg__SpawnedTurtle *
turtlesim_project_interfaces__msg__SpawnedTurtle__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  turtlesim_project_interfaces__msg__SpawnedTurtle * msg = (turtlesim_project_interfaces__msg__SpawnedTurtle *)allocator.allocate(sizeof(turtlesim_project_interfaces__msg__SpawnedTurtle), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(turtlesim_project_interfaces__msg__SpawnedTurtle));
  bool success = turtlesim_project_interfaces__msg__SpawnedTurtle__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
turtlesim_project_interfaces__msg__SpawnedTurtle__destroy(turtlesim_project_interfaces__msg__SpawnedTurtle * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    turtlesim_project_interfaces__msg__SpawnedTurtle__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__init(turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  turtlesim_project_interfaces__msg__SpawnedTurtle * data = NULL;

  if (size) {
    data = (turtlesim_project_interfaces__msg__SpawnedTurtle *)allocator.zero_allocate(size, sizeof(turtlesim_project_interfaces__msg__SpawnedTurtle), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = turtlesim_project_interfaces__msg__SpawnedTurtle__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        turtlesim_project_interfaces__msg__SpawnedTurtle__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__fini(turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      turtlesim_project_interfaces__msg__SpawnedTurtle__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence *
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * array = (turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence *)allocator.allocate(sizeof(turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__destroy(turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__are_equal(const turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * lhs, const turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!turtlesim_project_interfaces__msg__SpawnedTurtle__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence__copy(
  const turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * input,
  turtlesim_project_interfaces__msg__SpawnedTurtle__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(turtlesim_project_interfaces__msg__SpawnedTurtle);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    turtlesim_project_interfaces__msg__SpawnedTurtle * data =
      (turtlesim_project_interfaces__msg__SpawnedTurtle *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!turtlesim_project_interfaces__msg__SpawnedTurtle__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          turtlesim_project_interfaces__msg__SpawnedTurtle__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!turtlesim_project_interfaces__msg__SpawnedTurtle__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
